...
const GameLogic = (() => {
  ...
  /* ── Monde 5 : Mémoire — mélodie séquentielle ── */
  function generateWorld5() {
    const length = 3 + Math.floor(GameState.get().xp / 300); // augmente avec XP
    const baseRoot = randomRoot();
    const scale = [0,2,4,5,7,9,11,12]; // gamme majeure
    const melody = Array.from({length}, () =>
      baseRoot + scale[Math.floor(Math.random() * scale.length)]
    );
    // Créer des variantes dont une est la bonne
    const variants = generateMelodyVariants(melody, 4);
    return {
      world: 5,
      melody,
      answer: 0,
      ...
      play: () => {
        AudioEngine.init();
        const now = AudioEngine.getCtx().currentTime + 0.05;
        melody.forEach((note, i) => AudioEngine.playNote(note, now + i * 0.5, 0.9));
      },
  ...
  function generateMelodyVariants(original, count) {
    const variants = [original];
    const scale = [0,2,4,5,7,9,11,12];
    const base = original[0] - (original[0] % 12);
    for (let i = 1; i < count; i++) {
      const v = original.map((n, idx) => {
        if (Math.random() < 0.4) {
          return base + scale[Math.floor(Math.random() * scale.length)];
        }
        return n;
      });
      variants.push(v);
...
const UI = (() => {
  ...
  function nextExercise() {
    ...
    if (ex.world === 4) {
      ...
      document.getElementById('root-note-label').textContent = ex.rootName;
    } else {
  ...
  function playCurrentExercise() {
    ...
    const delay = ex.world === 5 ? (ex.melody.length * 500 + 400) : 1600;
    setTimeout(() => {
      if (!answered) {
        playBtn.classList.remove('playing');
